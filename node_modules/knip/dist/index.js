...
export const main = async (unresolvedConfiguration) => {
    ...
    const internalWorkspaceFilePaths = new Set();
    const handleReferencedDependency = getHandler(collector, deputy, chief);
    const updateImports = (importedModule, importItems) => {
        for (const id of importItems.identifiers)
            importedModule.identifiers.add(id);
        for (const id of importItems.importedNs)
            importedModule.importedNs.add(id);
        for (const id of importItems.isReExportedBy)
            importedModule.isReExportedBy.add(id);
        for (const id of importItems.isReExportedNs)
            importedModule.isReExportedNs.add(id);
    ...
    const updateImported = (filePath, importItems) => {
        ...
        if (!importedFile.imported)
            importedFile.imported = importItems;
        else
            updateImports(importedFile.imported, importItems);
    };
    const setInternalImports = (filePath, internalImports) => {
        for (const [specifierFilePath, importItems] of Object.entries(internalImports)) {
            ...
            if (!file.imports.internal[specifierFilePath])
                file.imports.internal[specifierFilePath] = importItems;
            else
                updateImports(file.imports.internal[specifierFilePath], importItems);
