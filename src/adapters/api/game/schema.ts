...
export type BoardInDB = ({ id: string, isNotMoved?: boolean, turnMovedTwo?: number } | undefined)[][]
...
export const convertEntityToBoardInDB = (board: Board): BoardInDB =>
    board.map((row) => row.map((mass) => {
        if (mass == null) return undefined
        if (mass instanceof Pawn) {
            return { id: mass.id, isNotMoved: mass.isNotMoved, turnMovedTwo: mass.turnMovedTwo }
        }
        if (mass instanceof King || mass instanceof Rook) {
            return { id: mass.id, isNotMoved: mass.isNotMoved, turnMovedTwo: undefined }
        }
        return { id: mass.id, isNotMoved: undefined, turnMovedTwo: undefined }
    }))

export const convertBoardInDBToEntity = (board: BoardInDB): Board => board.map((row) => row.map((mass) => {
    if (mass == null) return undefined
