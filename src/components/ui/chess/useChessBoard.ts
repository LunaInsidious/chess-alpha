...
import { reverseColor, getMyPiecePositionsByType, findEscapeMoves, isChecked, getSafeMovablePositions, checkDraw } from "@/domains/piece/common";
...
export const useChessBoard = ({ playerColor }: ChessBoardHookProps) => {
    const [boardStatus, setBoardStatus] = useState<BoardStatus>({ board: Array(8).fill(Array(8).fill(undefined)), turn: 0, fiftyMoveRuleTurn: 0 });

    const { showError, showAlert } = useAlert();

    const navigate = useNavigate()

    const [selectedPiecePosition, setSelectedPiecePosition] = useState<Position | undefined>(undefined);
    ...
    const moveCallback = async (board: BoardStatus, kingPosition: Position, myColor: 'white' | 'black', isPlayer: boolean): Promise<{
        isChecked: boolean,
        isGameEnd: boolean,
        ...
        if (isChecked(board.board, kingPosition, myColor, isPlayer)) {
            const escapeMoves = findEscapeMoves(board, kingPosition, myColor, isPlayer);
        ...
        const drawStatus = await checkDraw(board, kingPosition, myColor, isPlayer, gameDataAPI.countByBoard);
        if (drawStatus != null) {
            setGameStatus({
                player: drawStatus,
                enemy: drawStatus
    ...
    const handleSelectPiece = (position: Position) => {
        ...
        if (piece != null) {
            ...
            if (selectedPiecePosition != null) {
                ...
                if (selectedPiecePosition.x === position.x && selectedPiecePosition.y === position.y) return;
                // 自分の駒を選択した場合は、アラートを表示して何もしない
                showAlert({
                    severity: "warning",
                    message: "touch moveルールにより、選択した駒を選び直すことはできません。",
                    timeout: 3000
                })
                return
            }
            ...
            if (gameStatus.player === "checked") {
                const escapeMoves = findEscapeMoves(boardStatus, position, playerColor, true);
                if (escapeMoves.length <= 0) {
                    showAlert({
                        severity: "warning",
                ...
                const validMoves = getSafeMovablePositions(boardStatus, piece, position, true);
    ...
    const handleMovePiece = async (to: Position) => {
        ...
        const piece = boardStatus.board[selectedPiecePosition.y][selectedPiecePosition.x];
        if (piece == null) {
            throw new Error('選択した駒が存在しません。');
        }
        const newBoardStatus = piece.move(boardStatus, selectedPiecePosition, to, true);
        ...
        const enemyKingPositions = getMyPiecePositionsByType(newBoardStatus, enemyColor, "K")
        ...
        if (isPromotionMoved) {
            setPromotionInfo({
                ...
                callback: async (boardOnCallback: Board) => {
                    const { isChecked: isEnemyChecked, isGameEnd, escapeMoves: enemyEscapeMoves } = await moveCallback({ ...newBoardStatus, board: boardOnCallback }, enemyKingPosition, enemyColor, false);
                    setPromotionInfo(undefined);
                    if (isGameEnd) return

                    setIsPlayerTurn(false);
                    setTimeout(() => {
                        cpuHook.cpuMove({ ...newBoardStatus, board: boardOnCallback }, isEnemyChecked, enemyEscapeMoves);
        ...
        const { isChecked: isEnemyChecked, isGameEnd, escapeMoves: enemyEscapeMoves } = await moveCallback(newBoardStatus, enemyKingPosition, enemyColor, false);
        if (isGameEnd) return
    ...
    const handleClickMass = (position: Position) => {
        if (movablePositions.some(pos => pos.x === position.x && pos.y === position.y)) {
    ...
    const handleClickPromotion = (type: "Queen" | "Rook" | "Bishop" | "Knight") => {
        ...
        const typeDict: Record<"Queen" | "Rook" | "Bishop" | "Knight", "Q" | "R" | "B" | "N"> = {
            Queen: "Q",
            Rook: "R",
            Bishop: "B",
            Knight: "N"
        }
        const newBoard = promotion(boardStatus.board, typeDict[type], piece, mass, playerColor);
        ...
        (async () => {
            try {
                ...
                if (latestBoardStatus == null) {
                    ...
                    const isPlayerTurnInLatest = (playerColor === "white" && latestBoardStatus.gameData.turn % 2 === 0) || (playerColor === "black" && latestBoardStatus.gameData.turn % 2 === 1);
                    setIsPlayerTurn(isPlayerTurnInLatest);

                    // プレイヤーのターンでない場合はCPUの処理を行う
                    if (!isPlayerTurnInLatest) {
                        const enemyColor = reverseColor(playerColor);
                        const enemyKingPositions = getMyPiecePositionsByType(latestBoardStatus.gameData, enemyColor, "K")
                        if (enemyKingPositions.length <= 0) {
                            throw new Error('敵のキングが存在しません。');
                        }
                        const enemyKingPosition = enemyKingPositions[0];
                        const { isChecked: isEnemyChecked, isGameEnd, escapeMoves: enemyEscapeMoves } = await moveCallback(latestBoardStatus.gameData, enemyKingPosition, enemyColor, false);
                        if (isGameEnd) return

                        setTimeout(() => {
                            cpuHook.cpuMove(latestBoardStatus.gameData, isEnemyChecked, enemyEscapeMoves);
                            setIsPlayerTurn(true);
                        }, 1000);
                    }
                }
