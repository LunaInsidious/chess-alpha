...
const calcMove = (current: number, moveDistance: number, isPlayer: boolean) =>
  isPlayer ? current + moveDistance : current - moveDistance;
...
export const promotion = (
  ...
  switch (type) {
    ...
    case "R":
      ...
      break;
    case "B":
      newBoard[position.y][position.x] = new Bishop(
        `${myColor[0]}B${piece.id}`,
      );
      break;
    case "N":
      newBoard[position.y][position.x] = new Knight(
        `${myColor[0]}N${piece.id}`,
      );
      break;
    default:
      break;
...
const getPawnMovablePositions = (
  ...
  // 斜め前に敵がいる場合、その位置に進める
  if (
    isEnemyExist(boardStatus, { x: from.x + 1, y: oneVerticalMove }, enemyColor)
  ) {
...
export class Pawn extends Piece {
  ...
  getMovablePositions = (
    ...
    const movablePositions = getPawnMovablePositions(
      boardStatus,
      this,
      from,
      isPlayer,
    ...
    if (from.x < 7) {
      ...
      if (
        rightMass instanceof Pawn &&
        rightMass.turnMovedTwo === boardStatus.turn - 1
      ) {
    ...
    if (from.x > 0) {
      ...
      if (
        leftMass instanceof Pawn &&
        leftMass.turnMovedTwo === boardStatus.turn - 1
      ) {
        movablePositions.push({
          x: from.x - 1,
          y: from.y,
