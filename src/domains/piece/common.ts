import { Board, BoardStatus, Piece, PiecePosition, PieceType, Position } from "@/domains/piece/piece";

// whiteまたはblackを受け取り、逆の色を返す
export const reverseColor = (color: 'white' | 'black'): 'white' | 'black' => (color === 'white' ? 'black' : 'white');

export const getMyPieces = (board: Board, myColor: 'white' | 'black'): PiecePosition[] => {
    const myPieces: {
        piece: Piece;
        position: Position;
    }[] = [];
...
export const getMyPiecePositionsByType = (boardStatus: BoardStatus, myColor: 'white' | 'black', type: PieceType): Position[] => {
...
// 自身がチェックされているかどうかを確認。
export const isChecked = (board: Board, kingPosition: Position, myColor: 'white' | 'black', isPlayer: boolean): boolean => {
    ...
    for (let i = 1; kingPosition.x + i < 8 && kingPosition.y + i < 8; i++) {
        ...
        if (targetMass != null) {
            if (targetMass.color === myColor) break
            const firstMassCheck = isPlayer ? i === 1 && (targetMass.type === 'P' || targetMass.type === "K") : i === 1 && targetMass.type === 'K'
    ...
    for (let i = 1; kingPosition.x + i < 8 && kingPosition.y - i >= 0; i++) {
        ...
        if (targetMass != null) {
            if (targetMass.color === myColor) break
            const firstMassCheck = isPlayer ? i === 1 && targetMass.type === "K" : i === 1 && (targetMass.type === 'K' || targetMass.type === 'P')
            if (firstMassCheck) return true;
            if (targetMass.type === 'B' || targetMass.type === 'Q') return true;
            break
        }
    ...
    for (let i = 0; i < knightMoves.length; i++) {
        ...
        if (kingPosition.x + move.x >= 0 && kingPosition.x + move.x < 8 && kingPosition.y + move.y >= 0 && kingPosition.y + move.y < 8) {
            const targetMass = board[kingPosition.y + move.y][kingPosition.x + move.x];
            if (targetMass != null && targetMass.color !== myColor && targetMass.type === 'N') return true;
        }
    }
...
export const getSafeMovablePositions = (boardStatus: BoardStatus, piece: Piece, from: Position, isPlayer: boolean): Position[] => {
    ...
    if (piece.type === "K") {
        const validMovesWithoutSuicide = validMoves.filter((move) => {
            ...
            const isCheck = !isChecked(newBoardStatus.board, move, piece.color, isPlayer);
            return isCheck;
        })
    ...
    const validMovesWithoutSuicide = validMoves.filter((move) => {
        const newBoardStatus = piece.move(boardStatus, from, move, isPlayer);
        return !isChecked(newBoardStatus.board, kingPosition[0], piece.color, isPlayer);
    })
...
const checkIsStalemate = (boardStatus: BoardStatus, kingPosition: Position, myColor: 'white' | 'black', isPlayer: boolean): boolean => {
    ...
    return myPieces.every((myPiece) => {
        const movablePositions = getSafeMovablePositions(boardStatus, myPiece.piece, myPiece.position, isPlayer);
...
const checkIsUnderResourced = (board: Board): boolean => {
    ...
    // キング対キング＆ビショップ
    const whiteBishops = whitePieces.filter((whitePiece) => whitePiece.piece.type === 'B');
    const blackBishops = blackPieces.filter((blackPiece) => blackPiece.piece.type === 'B');
    if (whiteBishops.length === 0 && blackBishops.length === 0) return false;

    if ((whitePieces.length === 2 && blackPieces.length === 1) || (whitePieces.length === 1 && blackPieces.length === 2)) return true;

    // キング対キング＆ナイト
    const whiteKnights = whitePieces.filter((whitePiece) => whitePiece.piece.type === 'N');
    const blackKnights = blackPieces.filter((blackPiece) => blackPiece.piece.type === 'N');
    // 可読性を考慮して、if文を分割
    ...
    // キング＆ビショップ対キング＆ビショップ
    if (whiteBishops.length === 0 || blackBishops.length === 0) return false;
    const whiteBishopMassColors = whiteBishops.map((whiteBishop) => whiteBishop.position.x % 2 === whiteBishop.position.y % 2 ? 'black' : 'white');
    const blackBishopMassColors = blackBishops.map((blackBishop) => blackBishop.position.x % 2 === blackBishop.position.y % 2 ? 'black' : 'white');

    const isBishopInSameColorMass = whiteBishopMassColors.every((color) => blackBishopMassColors.includes(color));

    if (whitePieces.length === whiteBishops.length + 1 && blackPieces.length === blackBishops.length + 1 && isBishopInSameColorMass) return true;
    return false;
}

const checkIsThreefoldRepetition = async (board: Board, countFunc: (board: Board) => Promise<number>): Promise<boolean> => {
    try {
...
export const checkDraw = async (boardStatus: BoardStatus, kingPosition: Position, myColor: 'white' | 'black', isPlayer: boolean, countDuplicate: (board: Board) => Promise<number>): Promise<"stalemate" | "underResource" | "threefold" | "fiftyRule" | undefined> => {
    if (checkIsStalemate(boardStatus, kingPosition, myColor, isPlayer)) return "stalemate";
    if (checkIsUnderResourced(boardStatus.board)) return "underResource";
    if (await checkIsThreefoldRepetition(boardStatus.board, countDuplicate)) return "threefold";
    if (boardStatus.fiftyMoveRuleTurn >= 50) return "fiftyRule";
    return undefined
}

// 自身がチェックされている際、チェックメイトから逃れることができる手を返す。なければチェックメイト
export const findEscapeMoves = (boardStatus: BoardStatus, kingPosition: Position, myColor: 'white' | 'black', isPlayer: boolean): {
    piece: Piece;
    from: Position;
    to: Position;
}[] => {
    ...
    myPieces.forEach((myPiece) => {
        const movablePositions = myPiece.piece.getMovablePositions(boardStatus, myPiece.position, isPlayer);
...
export const movePiece = (boardStatus: BoardStatus, piece: Piece, from: Position, to: Position): BoardStatus => {
    const targetMass = boardStatus.board[to.y][to.x];
    const isFiftyMoveRuleTurnUpdate = piece.type === 'P' || (targetMass != null && targetMass.color !== piece.color);
    const newBoard = [...boardStatus.board.map((row) => [...row])]
    ...
    return {
        ...
        fiftyMoveRuleTurn: isFiftyMoveRuleTurnUpdate ? 0 : boardStatus.fiftyMoveRuleTurn + 1,
    };
}

// fromからdirection(縦、横、斜め)の方向に駒が存在するかどうかを確認。あればそこまでのPositionの配列を返す。
export const getMovablePositions = (boardStatus: BoardStatus, from: Position, direction: "vertical" | "horizontal" | "diagonal", playerColor: 'white' | 'black'): Position[] => {
