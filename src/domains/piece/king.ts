...
const getKingMovablePositions = (boardStatus: BoardStatus, piece: King, from: Position, isPlayer: boolean): Position[] => {
    const directions = [
    ...
    if (piece.isNotMoved) {
        ...
        if (rightRook instanceof Rook && rightRook.isNotMoved) {
            ...
            const canCastling = [1, 2, 3].every((x) => {
                ...
                return !isChecked(boardStatus.board, { x, y: from.y }, piece.color, isPlayer);
            });
            if (canCastling) {
...
export class King extends Piece {
    ...
    getMovablePositions = (boardStatus: BoardStatus, from: Position, isPlayer: boolean) => getKingMovablePositions(boardStatus, this, from, isPlayer);
        ...
        if (Math.abs(from.x - to.x) === 2) {
            ...
            if (to.x === 2) {
                ...
                if (rook instanceof Rook) {
                    const newBoardStatus = rook.move(boardStatus, { x: 0, y: from.y }, { x: 3, y: from.y });
                    // ターンはrookで更新された分を戻す
                    return movePiece({ ...newBoardStatus, turn: newBoardStatus.turn - 1 }, this, from, to);
                }
            }
            // キングサイドキャスリング
            if (to.x === 6) {
                ...
                if (rook instanceof Rook) {
                    const newBoardStatus = rook.move(boardStatus, { x: 7, y: from.y }, { x: 5, y: from.y });
                    return movePiece({ ...newBoardStatus, turn: newBoardStatus.turn - 1 }, this, from, to);
                }
            }
        }
