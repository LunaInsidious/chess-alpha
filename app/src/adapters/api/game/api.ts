import {
  GameDatabase,
  BoardStatusInDB,
  convertEntityToBoardInDB,
  convertBoardInDBToEntity,
...
export const useGameDataAPI = (): GameAPI => ({
  ...
  create: async (
    ...
    await db.game.put({
      ...
      turn: gameData.turn,
      fiftyMoveRuleTurn: gameData.fiftyMoveRuleTurn,
      playerColor,
    });
  },
  countByBoard: async (board: Board): Promise<number> => {
    ...
    return db
      .table("game")
      .filter(
        (boardStatus: BoardStatusInDB) => boardStatus.board === boardFormatted,
      )
      .count();
  },
  findLatest: async (): Promise<
